...
def get_todays_games():
    """Fetches today's games using daily_schedule."""
    ...
    try:
        ...
        for g in game_list:
            ...
            match = f"{away} @ {home}"
            games.append({"label": match, "teams": [away, home]})
        return games
    except Exception:
        return []
...
with st.sidebar:
    ...
    selected_game_label = st.selectbox("Filter by Today's Games", options=game_options)
    
    if selected_game_label != "All Teams":
