...
def get_all_active_players(season_str):
    ...
    try:
        ...
        for team in teams:
            ...
            # Loop through position groups and store the position code
            for group in ['forwards', 'defensemen', 'goalies']:
                for p in roster.get(group, []):
                    all_players.append({
                        ...
                        "team": abbr,
                        "pos": p.get('positionCode', group[0].upper()) # Extracting position code
                    })
    except: pass
...
with st.sidebar:
    ...
    # Modified label to include position to differentiate same-name teammates
